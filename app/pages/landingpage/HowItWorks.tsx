...
export default function HowItWorks() {
    ...
    return (
        <section className="w-full bg-ink py-20 sm:py-28">
            ...
            {/* ledger, full width */}
            <div className="relative w-full border-y border-dashed border-paper/10">
                <motion.div
                    aria-hidden
                    initial={{ scaleY: 0 }}
                ...
                {steps.map((s, i) => (
                    <motion.div
                        ...
                        className={`grid w-full grid-cols-[72px_1fr] border-dashed border-paper/10 sm:grid-cols-[96px_1fr] lg:grid-cols-[120px_1fr] ${i !== 0 ? "border-t" : ""
                            }`}
                    >
                        {/* marker */}
                        <div className="flex flex-col items-center justify-center gap-2 border-r border-dashed border-paper/10 py-10">
                            <div className="flex h-11 w-11 items-center justify-center rounded-full border-2 border-accent bg-ink text-accent">
                        ...
                        <div className="relative flex flex-col gap-2 overflow-hidden py-10 pl-6 pr-6 sm:pl-10 sm:pr-10 lg:flex-row lg:items-baseline lg:gap-10 lg:pl-12">
                            <span
                                aria-hidden
                                className="pointer-events-none absolute -right-4 top-1/2 hidden -translate-y-1/2 select-none font-bricolage text-[140px] font-extrabold leading-none text-paper/[0.04] xl:block"
                            >
                                {s.number}
                            </span>

                            <h3 className="shrink-0 font-bricolage text-2xl font-bold text-paper lg:w-64 lg:text-3xl">
                                {s.title}
                            </h3>
                            <p className="max-w-xl text-sm leading-relaxed text-paper/60 font-rethink sm:text-base lg:border-l lg:border-dashed lg:border-paper/10 lg:pl-10">
                                {s.desc}
                            </p>
                        </div>
                    </motion.div>
