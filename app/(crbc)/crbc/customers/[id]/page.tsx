...
export default async function CustomerProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 space-y-6">
      <Link href="/crbc/customers" className="inline-flex items-center gap-1.5 text-xs text-zinc-400 hover:text-zinc-700 transition-colors">
      ...
      <div className="bg-white border border-zinc-100 rounded-xl p-6">
        <div className="flex items-start justify-between">
          <div>
           ...
           <div className="flex items-baseline gap-x-1 mt-0.5">
            <span className="text-zinc-900 text-xs">Full Name:</span>
        ...
        <div className="mt-4 pt-4 border-t border-zinc-50 grid grid-cols-2 gap-3 text-xs">
          <div><span className="text-zinc-400">Email</span><p className="text-zinc-700 mt-0.5">{customer.email}</p></div>
          <div><span className="text-zinc-400">Registered</span><p className="text-zinc-700 mt-0.5">{formatDate(customer.created_at)}</p></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map(({ label, value }) => (
          <div key={label} className="bg-white border border-zinc-100 rounded-xl p-4">
            ...
            <p className="text-zinc-900 text-2xl font-semibold mt-2">{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white border border-zinc-100 rounded-xl">
        <div className="px-5 py-4 border-b border-zinc-100">
          <h2 className="text-zinc-800 text-sm font-medium">Shipment History</h2>
        </div>
        {shipments.length === 0 ? (
          <p className="px-5 py-8 text-center text-zinc-400 text-xs">No shipments found.</p>
        ) : (
